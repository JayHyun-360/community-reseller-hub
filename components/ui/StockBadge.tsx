...
export function StockBadge({ status, qty, className = "" }: StockBadgeProps) {
  const config = {
    available: {
    ...
    >
      {config.animate && (
        <span className="relative flex h-2 w-2">
          <span
            className={`animate-ping absolute inline-flex h-full w-full rounded-full ${config.dot} opacity-40`}
          ></span>
          <span
