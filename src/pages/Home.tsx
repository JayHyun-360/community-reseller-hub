...
export const Home: React.FC = () => {
  ...
  return (
    <div className="max-w-[1600px] mx-auto pb-24 md:pb-12 bg-white min-h-screen">
      ...
      <section className="pt-12 px-6">
        <div className="max-w-2xl px-2">
          <h1 className="text-4xl md:text-7xl font-black tracking-tight text-zinc-900 leading-[0.9]">
            Find your next favorite thing <span className="text-zinc-400">nearby.</span>
          </h1>
          <p className="mt-8 text-zinc-500 font-medium text-lg md:text-2xl max-w-lg leading-relaxed">
            NearByt connects you with local artisans and curators in a visual scroll of discovery.
          </p>
        </div>
        ...
        <div className="mt-16 overflow-x-auto pb-8 hide-scrollbar scroll-smooth">
          <div className="flex gap-8 px-2">
            {MOCK_PRODUCTS.slice(0, 6).map((item, idx) => (
              <div key={idx} className="flex-shrink-0 w-44 md:w-64 group cursor-pointer">
                ...
                <div className="px-2">
                  <p className="text-[10px] font-black uppercase tracking-widest text-zinc-400 mb-1">Trending</p>
                  <h4 className="text-sm font-bold text-zinc-900 truncate leading-none">{item.title}</h4>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Verified Sellers Circle Strip */}
      <div className="mt-8 px-6">
          <VerifiedSellersStrip sellers={MOCK_SELLERS} />
      </div>

      {/* Discover Section (Masonry) */}
      <section className="px-6 py-12">
        <div className="flex flex-col gap-10">
          {/* Section Header */}
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-2">
            <div className="space-y-1">
              <h2 className="text-2xl font-black text-zinc-900 tracking-tight">Today's Picks</h2>
              <p className="text-[11px] text-zinc-400 font-black uppercase tracking-[0.2em]">Curated from the neighborhood</p>
            </div>
            <div>
              <CategoryFilter 
          ...
          {filteredProducts.length === 0 && (
            <div className="py-40 text-center flex flex-col items-center gap-6">
              ...
              <p className="text-zinc-300 font-black uppercase tracking-[0.3em] text-[10px]">No finds in this category today</p>
            </div>
          )}
        </div>
      </section>
